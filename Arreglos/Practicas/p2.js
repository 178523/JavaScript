...
function calculate() {
    ...
    for (let i = 0; i < counterClicksAddElements; i++) {
        ...
        ArrayMin[i] = Arrayuno[i].toLowerCase();
        console.log(Arrayuno[i].charAt(0));
        if ((ArrayMin[i].charAt(0) == 'a') || (ArrayMin[i].charAt(0) == 'e') || (ArrayMin[i].charAt(0) == 'i') || (ArrayMin[i].charAt(0) == 'o') || (ArrayMin[i].charAt(0) == 'u')) {

            ArrayOK[i] = Arrayuno[i];
        } else {
            ArrayOK[i] = "----";
        }
    }

    MostrarElementos(ArrayOK);
